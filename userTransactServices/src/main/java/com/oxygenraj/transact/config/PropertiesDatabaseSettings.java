...
final class PropertiesDatabaseSettings {
    ...
    static final String PREFIX = "user-transact.properties-datasource.";
    ...
    static PropertiesDatabaseSettings load(ConfigurableEnvironment environment) {
        ...
        try {
            ...
            throw new IllegalStateException("Invalid properties database configuration. Resolve EUREKA_DB_URL and "
                    + "EUREKA_DB_PASSWORD, and set user-transact.properties-datasource.username to EUREKA_DB.");
        }

        if (!"EUREKA_DB".equals(username)) {
            throw new IllegalStateException("user-transact.properties-datasource.username must be EUREKA_DB.");
        }
